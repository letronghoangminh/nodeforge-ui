...
const SubscriptionCard = ({ data, isAdmin }: SubscriptionCardProps) => {
  ...
  const onHandleUpgrade = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://api.nodeforge.site/api/subscription/pro-subscription`,
        {
          method: "GET",
          headers: {
  ...
  const onHandleManage = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await fetch(
        `https://api.nodeforge.site/api/subscription/portal-session`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    <Card className="w-[1200px]">
      ...
      <CardContent className=" space-y-6">
        <div className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
          ...
          <Badge
            variant={
              data?.type === SubscriptionType.FREE ? "success" : "warning"
            }
          >
            {data?.type}
          </Badge>
        ...
        <div className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
          ...
          >
            {data?.type === SubscriptionType.FREE ? "2" : "5"}
          </Badge>
        </div>
      </CardContent>
      <CardFooter>
          ...
          <div className="flex justify-end w-full gap-4">
            {data?.type === SubscriptionType.FREE && (
              <Button
                ...
                type="button"
                variant={
                  data?.type === SubscriptionType.FREE ? "green" : "warning"
                }
              >
                Upgrade Plan
              </Button>
            )}
            <Button onClick={() => onHandleManage()}>Manage Plan</Button>
          </div>
        )}
      </CardFooter>
    </Card>
